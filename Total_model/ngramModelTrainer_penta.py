...
# keith
#How to use
# ngramModelTrainer_penta.py --input_corpus 'google10000.txt' --compute_tetragrams
...
if __name__ == "__main__":
    ...
    parser.add_argument('--alphabet', required=False, choices=['almazan36', 'dummy', 'dutta_extended', 'sophia','atoz'], default='atoz', help='Alphabet to be used.')
    ...
    if args.input_corpus != '':
        ...
        if args.compute_tetragrams:
            ...
            np.save( 'savedNgram/quadgrams', savedict['quadgrams'])
            np.save( 'savedNgram/pentagrams', savedict['pentagrams'])

    #    scipy.io.savemat(basename(filename)+'.ngrams.mat', savedict)
        np.save( 'savedNgram/alphabet', alphabet)
        np.save( 'savedNgram/unigrams', unigramsPdf)
        np.save( 'savedNgram/bigrams', savedict['bigrams'])
        np.save( 'savedNgram/trigrams', savedict['trigrams'])
    else:
        unittest.main()
